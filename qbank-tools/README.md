...
- `process-screenshot-batch` also **hard-excludes qids already localized for the language** (keys under `.questions` in production `translations.<lang>.json`) from the candidate pool, so two screenshots in one language can't both map to the same master question. It is **per-language** and **ON by default**; disable with `--exclude-claimed false` (needed only when intentionally re-matching an already-merged batch). Excluded qids are not dropped silently: each item records `analysis.claimedQidExclusion`, and when the best match was a claimed qid the item gets the `claimed-qid-duplicate-suspect` reason code so genuine target-language duplicates surface for review. The run report lists `claimedQidExclusion.duplicateSuspectItems`.

- `process-screenshot-batch` also **drops confirmed in-app verbatim-duplicate screenshots** before matching (a foreign app showing the *same* question at two positions). The registry `qbank-tools/history/duplicate-exclusions.<lang>.json` lists screenshot basenames to skip; the matcher honors it **ON by default** (disable with `--exclude-duplicates false`). The run report records `duplicateExclusion.excludedItems` (with the group id and which copy was kept) and `intakeStats.duplicateItemsSkipped`. Build the registry with **`npm run apply-duplicate-decisions -- --lang <lang> --path <export.json>`**, which ingests a reviewed export from the standalone duplicate-review workbench (see below). It only deletes screenshots from groups the reviewer marked `verdict:"dup"`; `not`/`unsure` groups are ignored. Idempotent (`--dry-run` to preview).

  - The duplicate-review **workbench** is generated separately from the per-batch workbench: an all-screenshots verbatim scan (crop out the number-strip/counter → tesseract OCR → group identical prompt+options → image-diff each group → side-by-side montages) produces `imports/<lang>/_dupscan/<lang>-duplicates-workbench.html`. The reviewer clicks `dup`/`unsure`/`not` per group and exports `<lang>-duplicates-decisions.json`. **Key trap (why a human/image step is required):** sign and dashboard questions frequently share an *identical prompt + identical answer options* but a **different image** (mirrored arrow, different sign, different route plate) — these are **not** duplicates and must never be deleted; only text-plus-image agreement counts.
